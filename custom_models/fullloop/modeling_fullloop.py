...
class FullLoopPreTrainedModel(GatedDeltaNetPreTrainedModel):
...
class FullLoopModel(FullLoopPreTrainedModel):
    ...
    def __init__(self, config: FullLoopConfig):
        ...
        self.layers = nn.ModuleList(
            FullLoopBlock(config, i) for i in range(config.num_hidden_layers)
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[Any],
        ...
        for loop_idx in range(self.loop_count):
            ...
            if self.residual_weight is not None:
                hidden_states = hidden_states + self.residual_weight[loop_idx] * h_input
...
class FullLoopForCausalLM(FullLoopPreTrainedModel, GenerationMixin):

    _tied_weights_keys = ['lm_head.weight']

    def __init__(self, config: FullLoopConfig):
        super().__init__(config)
        self.model = FullLoopModel(config)
        self.vocab_size = config.vocab_size
