...
from .layers import FFNLoopBlock
...
class FFNLoopPreTrainedModel(GatedDeltaNetPreTrainedModel):
    config_class = FFNLoopConfig
    base_model_prefix = 'model'
    supports_gradient_checkpointing = True
    _no_split_modules = ['FFNLoopBlock']
    _supports_cache_class = False

    def _init_weights(self, module: nn.Module):
        super()._init_weights(module)
        if isinstance(module, GatedMLP) and next(module.parameters()).device.type != 'meta':
...
class FFNLoopModel(FFNLoopPreTrainedModel):
    def __init__(self, config: FFNLoopConfig):
        ...
        self.layers = nn.ModuleList(
            FFNLoopBlock(config, i) for i in range(config.num_hidden_layers)
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[Any],
        ...
        if output_attentions:
            warnings.warn('FFNLoopModel does not support output_attentions; ignoring.')
...
class FFNLoopForCausalLM(FFNLoopPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ['lm_head.weight']

    def __init__(self, config: FFNLoopConfig):
        super().__init__(config)
        self.model = FFNLoopModel(config)
        self.vocab_size = config.vocab_size
