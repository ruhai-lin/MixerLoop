    ...
    def __call__(self, examples: List[Union[List[int], Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if not self.varlen:
            if all('labels' in example for example in examples):
                input_lengths = {example['input_ids'].size(0) for example in examples}
                label_lengths = {example['labels'].size(0) for example in examples}
                if len(input_lengths) != 1 or input_lengths != label_lengths:
                    raise ValueError('Pretokenized input_ids and labels must have one shared fixed length')
...
def build_dataset(
    ...
    if dataset.lower() in {"climbmix", CLIMBMIX_REPOSITORY_ID.lower()}:
        return ClimbMixDataset(
            data_dir=data_dir,
...
def build_dataloader(
    ...
    if isinstance(dataset, ClimbMixDataset):
        dataset.configure(
            seq_len=seq_len,
            rank=rank,
